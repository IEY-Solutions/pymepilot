...
### Operativo
- **Auth frontend hardening:** el middleware activo ya valida rutas protegidas con `getUser()` también en prefetch/actions, el recovery flow quedó en `/forgot-password` → `/auth/callback` → `/reset-password`, sin cookie recovery custom, y la política mínima de password quedó alineada a 6 caracteres con tests y docs consistentes.
