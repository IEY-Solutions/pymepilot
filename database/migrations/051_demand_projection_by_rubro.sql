-- =============================================================================
-- Migracion 051: Reemplazar get_demand_projection para incluir rubro y sin limite
-- =============================================================================
-- Que hace: Devuelve TODOS los productos con ventas, agrupados por rubro derivado
--   del nombre del producto (Contabilium no pobla category).
-- =============================================================================
...
SELECT
    ...
    -- Rubro derivado del nombre
    CASE
        WHEN p.name ILIKE 'MAGCASE%' THEN 'Fundas'
        WHEN p.name ILIKE 'FUNDA + WALLET%' THEN 'Fundas + Wallet'
        WHEN p.name ILIKE 'BATERÍA%' OR p.name ILIKE 'BATERIA%' THEN 'Baterias'
        WHEN p.name ILIKE 'CARGADOR INALÁMBRICO%' OR p.name ILIKE 'CARGADOR INALAMBRICO%' THEN 'Cargadores inalambricos'
        WHEN p.name ILIKE 'CARGADOR DE AUTO%' THEN 'Cargadores de auto'
        WHEN p.name ILIKE 'CARGADOR DE PARED%' THEN 'Cargadores de pared'
        WHEN p.name ILIKE 'ESTACIÓN%' OR p.name ILIKE 'ESTACION%' THEN 'Estaciones de carga'
        WHEN p.name ILIKE 'SOPORTE%' OR p.name ILIKE 'SOPAPA%' THEN 'Soportes'
        WHEN p.name ILIKE 'COMBO%' THEN 'Combos'
        WHEN p.name ILIKE 'BILLETERA%' OR p.name ILIKE 'MAGRING%' THEN 'Accesorios'
        ELSE 'Otros'
    END AS product_rubro,
    ROUND(COALESCE(
...
ORDER BY product_rubro, projected_demand_30d DESC NULLS LAST
LIMIT p_limit;
$$;

COMMENT ON FUNCTION public.get_demand_projection IS
    'Todos los productos con ventas, agrupados por rubro, con demanda proyectada a 30 dias.';
